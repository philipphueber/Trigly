...
class NotificationPostedTriggerFactory : TriggerFactory {
    ...
    // SharedPayloadKeys.NOTIFICATION_KEY is emitted but deliberately not declared
    // here. It is opaque, generated by the posting app, and useful only to
    // dismiss_notification and notification_button, which read it directly.
    // Offering it in a picker would promise a person a value they cannot read.
    // See VariableSpec's KDoc.
    override val variables = listOf(
        VariableSpec(
            key = NotificationPostedTrigger.PAYLOAD_PACKAGE,
            label = "Package",
            kind = VariableKind.PACKAGE,
            sample = "com.example.chat",
        ),
        VariableSpec(
            key = NotificationPostedTrigger.PAYLOAD_TITLE,
            label = "Title",
        ...
        VariableSpec(
            key = NotificationPostedTrigger.PAYLOAD_TEXT,
            label = "Text",
            kind = VariableKind.TEXT,
            sample = "Are we still on for tonight?",
            alwaysPresent = false,
        ),
    )
...
class DndModeTriggerFactory : TriggerFactory {
    ...
    override val variables = listOf(
        VariableSpec(
            key = DndModeTrigger.PAYLOAD_STATE,
            label = "State",
            kind = VariableKind.STATE,
            sample = DndModeTrigger.ON,
            help = "One of '${DndModeTrigger.ON}' or '${DndModeTrigger.OFF}'.",
        ),
    )
