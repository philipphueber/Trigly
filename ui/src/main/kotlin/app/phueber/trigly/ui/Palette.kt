...
/*
 * `hex` used to live here, and it moved to `Hex.kt` for a reason that is not
 * visible from either file on its own. See that file's own comment: a top
 * level function in *this* file, called from [Tone]'s initialisers, makes
 * touching [Tone] initialise the schemes below out of a [Tone] that is not
 * finished yet.
 */
